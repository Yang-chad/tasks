...
BASE = Path("C:/Users/gree/Desktop/AI工作场景/项目管理")
...
def update_summary_bar(content, base_dir):
    ...
    for color, json_file, label, is_overdue in specs:
        ...
        if is_overdue:
            ...
            summary = jdata.get("summary", {"wait": 0, "doing": 0, "changed": 0, "other": 0})
            new_num = str(total)
            wait = summary.get("wait", 0)
            doing = summary.get("doing", 0)
            changed = summary.get("changed", 0)
            other = summary.get("other", 0)
            label_parts = [f"待{wait}", f"做{doing}"]
            if changed > 0:
                label_parts.append(f"变{changed}")
            if other > 0:
                label_parts.append(f"其他{other}")
            new_label = f'{emoji_map[label]} {label} ({"/".join(label_parts)})'

        # Match: <div class="num" id="..." style="color:COLOR;">OLD</div> (id optional)
        num_pattern = re.compile(
            rf'(<div class="num"[^>]*?style="color:{re.escape(color)};"[^>]*?>).+?(</div>)'
        )
        content, n1 = num_pattern.subn(rf'\g<1>{new_num}\g<2>', content, count=1)

        # Match label immediately after the num div
...
def update_card_descriptions(content, base_dir):
    ...
    card_specs = [
        ...
        # 迭代执行进展：动态统计当日目录中的迭代报告数量
        ("迭代执行进展</span>",  None,                            "{} 个进行中迭代<br>报告总览 →",               "_iter_count",     False),
    ]

    updated = 0
    for name_hint, json_file, fmt, total_key, use_overdue_ppl in card_specs:
        # ── 迭代执行进展特殊处理：统计迭代报告HTML数量 ──
        if total_key == "_iter_count":
            today_dir = base_dir / date.today().isoformat()
            iter_files = list(today_dir.glob("迭代*_执行进展报告.html")) if today_dir.exists() else []
            iter_count = len(iter_files)
            new_desc = fmt.format(iter_count)
            pattern = re.compile(
            ...
            if n > 0:
                updated += 1
            print(f"  [cards] 迭代执行进展: {iter_count} 个进行中迭代")
            continue
...
def update_card_date(content, today_str):
    ...
    # Also update workload card month suffix (e.g. _202606 → _202607)
    month_slug = today_str[:4] + today_str[5:7]  # "2026-07-01" → "202607"
    workload_pattern = re.compile(r'(团队产出效率分析_)\d{6}(\.html)')
    m = workload_pattern.search(content)
    if m and m.group(0) != f"团队产出效率分析_{month_slug}.html":
        content = workload_pattern.sub(rf'\g<1>{month_slug}\g<2>', content)
        print(f"  [links] Workload card: {m.group(0)} → 团队产出效率分析_{month_slug}.html")
...
def update_index_html(today_str, data, index_path):
    ...
    # ── Step 3: Prune to last 15 days ──
    from datetime import date, timedelta
    cutoff = (date.today() - timedelta(days=14)).isoformat()
    all_rows = list(re.finditer(
        r'<tr(?:\s+class="today")?\s*>\s*<td>(\d{4}-\d{2}-\d{2})</td>.*?</tr>',
...
def main():
    ...
    # 若昨日目录不存在，回溯到最近有数据的目录
    if not yesterday_json_dir.exists():
        d = yesterday - timedelta(days=1)
        while d >= today - timedelta(days=30):
            candidate = BASE / d.isoformat()
            if candidate.exists():
                yesterday_str = d.isoformat()
                yesterday_json_dir = candidate
                print(f"   ⚠️ 昨日目录 {BASE / yesterday_str} 不存在，回溯到 {yesterday_str}")
                break
            d -= timedelta(days=1)
        else:
            print(f"   ⚠️ 警告: 过去30天内无有效基线目录，变化数可能失真")
